...
func (a ByGuid) Len() int           { return len(a) }
...
func mainPage(cells map[string]*auctionrunner.SerializableCellState) string {
	...
	for _, cell := range cells {
		sortedCells = append(sortedCells, cell)
	}
			...
			if(destinationObj){
				if(destinationObj.tagName=='UL'){
					destinationObj.appendChild(contentToBeDragged);
						post('assignment', {slave_id: destinationObj.id, task_id: contentToBeDragged.id}, "get");
				}else{
					destinationObj.parentNode.insertBefore(contentToBeDragged,destinationObj);
						post('assignment', {slave_id: destinationObj.parentNode.id, task_id: contentToBeDragged.id}, "get");
				}
...
<body>
	...
	for _, cell := range sortedCells {
		...
		pageStr += fmt.Sprintf(`
  		<div>
  			...
  			<ul id="%s">
		`, cellId[len(cellId) - 4:], cell.AvailableResources.Containers, cell.AvailableResources.MemoryMB, cellId)
  ...
  <div id="dragDropIndicator"><img src="http://i.imgur.com/LMz4u7o.gif"></div>
